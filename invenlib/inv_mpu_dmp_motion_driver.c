...
__code const uint8_t dmp_memory[DMP_CODE_SIZE] =
{
    /* bank # 0 */
...
#ifdef EMPL_NO_64BIT
	// sdcc does not support long long constants
	{
	float ll_constant = 32768;
	ll_constant *= ll_constant;
	
    gyro_bias_body[0] = (long)(((float)gyro_bias_body[0] * GYRO_SF) / ll_constant);
    gyro_bias_body[1] = (long)(((float)gyro_bias_body[1] * GYRO_SF) / ll_constant);
    gyro_bias_body[2] = (long)(((float)gyro_bias_body[2] * GYRO_SF) / ll_constant);
	}
#else
...
/*
void dmp_set_shake_reject_thresh_sdcc(void)
{
	unsigned char tmp[4];
	tmp[0] = 0x00;
	tmp[1] = 0x8e;
...
void dmp_enable_feature(unsigned short mask)
{
	...
	/*
        ...
        dmp_set_shake_reject_thresh_sdcc();
