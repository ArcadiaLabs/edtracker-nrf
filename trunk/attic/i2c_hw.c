...
//#define I2C_ISSUE_START_COND	{ W2CON0 |= _BV(X_START); while (W2CON0 & _BV(X_START)); }
//#define I2C_ISSUE_STOP_COND		{ W2CON0 |= _BV(X_STOP); while (W2CON0 & _BV(X_STOP)); }
...
bool i2c_read(uint8_t reg_addr, uint8_t data_len, uint8_t *data_ptr)
{
	...
	if ((w2_status & _BV(NACK)) == 0)
	{
		...
		if (i2c_init_transfer(MPU_ADDR_READ))
		{
			...
			// This situation (NACK received on bus while trying to read
			// from a slave) leads to a deadlock in the 2-wire interface. 
